...
import { getApiKey, setApiKey, queryGemini, analyzeFloorPlanWithAI } from './gemini.js';

let currentVisionImageBase64 = null;
let currentVisionImageWidth = 1024;
let currentVisionImageHeight = 768;
let chatHistory = [];
...
function generateDetailedProject3Data(floors) {
    const rooms = [];
    
    // Central core / Lobby corridor
    rooms.push({
        name: 'Central Corridor',
        type: 'corridor',
        x: 0,
    ...
    // Wing A (Top-Left)
    rooms.push({ name: 'Apt A1 Bedroom 1', type: 'bedroom', x: -23.5, z: -16.5, width: 6.0, depth: 5.0 });
    rooms.push({ name: 'Apt A1 Bedroom 2', type: 'bedroom', x: -16.5, z: -16.5, width: 6.0, depth: 5.0 });
    rooms.push({ name: 'Apt A1 Toilet', type: 'toilet', x: -25.5, z: -12.5, width: 2.0, depth: 3.0 });
    rooms.push({ name: 'Apt A1 Kitchen', type: 'kitchen', x: -21.5, z: -12.5, width: 5.0, depth: 3.0 });
    rooms.push({ name: 'Apt A1 Living Room', type: 'living', x: -15.5, z: -8.5, width: 7.0, depth: 5.0 });
    rooms.push({ name: 'Apt A1 Lobby', type: 'room', x: -22.5, z: -8.5, width: 6.0, depth: 5.0 });

    // Wing A (Bottom-Left)
    rooms.push({ name: 'Apt A2 Bedroom 1', type: 'bedroom', x: -23.5, z: 16.5, width: 6.0, depth: 5.0 });
    rooms.push({ name: 'Apt A2 Bedroom 2', type: 'bedroom', x: -16.5, z: 16.5, width: 6.0, depth: 5.0 });
    rooms.push({ name: 'Apt A2 Toilet', type: 'toilet', x: -25.5, z: 12.5, width: 2.0, depth: 3.0 });
    rooms.push({ name: 'Apt A2 Kitchen', type: 'kitchen', x: -21.5, z: 12.5, width: 5.0, depth: 3.0 });
    rooms.push({ name: 'Apt A2 Living Room', type: 'living', x: -15.5, z: 8.5, width: 7.0, depth: 5.0 });
    rooms.push({ name: 'Apt A2 Lobby', type: 'room', x: -22.5, z: 8.5, width: 6.0, depth: 5.0 });

    // Wing A (Top-Right)
    rooms.push({ name: 'Apt A3 Bedroom 1', type: 'bedroom', x: 23.5, z: -16.5, width: 6.0, depth: 5.0 });
    rooms.push({ name: 'Apt A3 Bedroom 2', type: 'bedroom', x: 16.5, z: -16.5, width: 6.0, depth: 5.0 });
    rooms.push({ name: 'Apt A3 Toilet', type: 'toilet', x: 25.5, z: -12.5, width: 2.0, depth: 3.0 });
    rooms.push({ name: 'Apt A3 Kitchen', type: 'kitchen', x: 21.5, z: -12.5, width: 5.0, depth: 3.0 });
    rooms.push({ name: 'Apt A3 Living Room', type: 'living', x: 15.5, z: -8.5, width: 7.0, depth: 5.0 });
    rooms.push({ name: 'Apt A3 Lobby', type: 'room', x: 22.5, z: -8.5, width: 6.0, depth: 5.0 });

    // Wing A (Bottom-Right)
    rooms.push({ name: 'Apt A4 Bedroom 1', type: 'bedroom', x: 23.5, z: 16.5, width: 6.0, depth: 5.0 });
    rooms.push({ name: 'Apt A4 Bedroom 2', type: 'bedroom', x: 16.5, z: 16.5, width: 6.0, depth: 5.0 });
    rooms.push({ name: 'Apt A4 Toilet', type: 'toilet', x: 25.5, z: 12.5, width: 2.0, depth: 3.0 });
    rooms.push({ name: 'Apt A4 Kitchen', type: 'kitchen', x: 21.5, z: 12.5, width: 5.0, depth: 3.0 });
    rooms.push({ name: 'Apt A4 Living Room', type: 'living', x: 15.5, z: 8.5, width: 7.0, depth: 5.0 });
    rooms.push({ name: 'Apt A4 Lobby', type: 'room', x: 22.5, z: 8.5, width: 6.0, depth: 5.0 });

    // Wing C (Center-Top)
    rooms.push({ name: 'Apt C1 Bedroom', type: 'bedroom', x: -8.5, z: -15.5, width: 5.0, depth: 5.0 });
    rooms.push({ name: 'Apt C2 Bedroom', type: 'bedroom', x: 8.5, z: -15.5, width: 5.0, depth: 5.0 });
    rooms.push({ name: 'Apt C1 Living Room', type: 'living', x: -8.5, z: -9.5, width: 5.0, depth: 5.0 });
    rooms.push({ name: 'Apt C2 Living Room', type: 'living', x: 8.5, z: -9.5, width: 5.0, depth: 5.0 });
    rooms.push({ name: 'Apt C1 Toilet', type: 'toilet', x: -3.5, z: -12.5, width: 2.0, depth: 3.0 });
    rooms.push({ name: 'Apt C2 Toilet', type: 'toilet', x: 3.5, z: -12.5, width: 2.0, depth: 3.0 });

    // Wing B (Center-Bottom)
    rooms.push({ name: 'Apt B1 Bedroom', type: 'bedroom', x: -8.5, z: 15.5, width: 5.0, depth: 5.0 });
    rooms.push({ name: 'Apt B2 Bedroom', type: 'bedroom', x: 8.5, z: 15.5, width: 5.0, depth: 5.0 });
    rooms.push({ name: 'Apt B1 Living Room', type: 'living', x: -8.5, z: 9.5, width: 5.0, depth: 5.0 });
    rooms.push({ name: 'Apt B2 Living Room', type: 'living', x: 8.5, z: 9.5, width: 5.0, depth: 5.0 });
    rooms.push({ name: 'Apt B1 Toilet', type: 'toilet', x: -3.5, z: 12.5, width: 2.0, depth: 3.0 });
    rooms.push({ name: 'Apt B2 Toilet', type: 'toilet', x: 3.5, z: 12.5, width: 2.0, depth: 3.0 });
    ...
    return {
        ...
        source: "handcrafted_project3"
    };
}

/**
 * Encodes an HTML Image object to a base64 JPEG data URL.
 */
function getBase64FromImage(img, maxDim = 1024) {
    const canvas = document.createElement("canvas");
...
async function uploadImage(e) {
    ...
    try {
        ...
        const isProject3 = fileName.includes("1456245454") || fileName.toLowerCase().includes("project3") || fileName.toLowerCase().includes("project 3");
        ...
        if (isPlot98) {
            ...
            setStatus("Detected Project 3 Floor Plan. Loading detailed 3D structure...");
            buildingData = generateDetailedProject3Data(1);
            
            // Mock OCR data for Project 3
            const roomNames = buildingData.rooms.map(r => r.name);
            const dimensions = buildingData.rooms.map(r => `${r.width.toFixed(1)}x${r.depth.toFixed(1)}m`);
            const mockOcrData = {
                roomNames,
                dimensions,
                floorNumbers: ["1"],
                areaValues: buildingData.rooms.map(r => `${(r.width * r.depth).toFixed(1)} sq m`),
                fireSafetyLabels: ["Extinguisher located near core"],
                stairLabels: ["Main Staircase Center"],
                liftLabels: ["Double Elevator Core"],
                rawOCRText: "Project 3 1st floorplan details"
            };
            window.lastOCRData = mockOcrData;
            
            runOCR(ocrTargetFile);
            receiveBuildingData(buildingData);
        } else if (apiKey) {
            setStatus("Analyzing floor plan layout using Vision AI...");
            try {
                const base64Image = getBase64FromImage(image);
                buildingData = await analyzeFloorPlanWithAI(base64Image, currentVisionImageWidth, currentVisionImageHeight);
                
                // Populate window.lastOCRData from the AI structure so the measurements panel updates
                const roomNames = buildingData.rooms.map(r => r.name);
                const dimensions = buildingData.rooms.map(r => `${r.width.toFixed(1)}x${r.depth.toFixed(1)}m`);
                const mockOcrData = {
                    roomNames,
                    dimensions,
                    floorNumbers: [String(buildingData.floors || 1)],
                    areaValues: buildingData.rooms.map(r => `${(r.width * r.depth).toFixed(1)} sq m`),
                    fireSafetyLabels: [],
                    stairLabels: [],
                    liftLabels: [],
                    rawOCRText: "Parsed via Vision AI"
                };
                window.lastOCRData = mockOcrData;

                setStatus("Vision AI Analysis Complete. Generating 3D Twin...");
                receiveBuildingData(buildingData);
            } catch (aiErr) {
                console.error("Vision AI analysis failed, falling back to local analysis:", aiErr);
                setStatus("Vision AI Analysis failed. Falling back to local analysis...");
...
async function runOCR(target) {
    try {
        ...
        const isProject3Text = rawText.includes("1456245454") || rawText.toLowerCase().includes("project3") || rawText.toLowerCase().includes("project 3") || rawText.toLowerCase().includes("1st floorplan") || rawText.toLowerCase().includes("floorplan");
        ...
        if (isPlot98Text && (!window.lastBuildingData || window.lastBuildingData.source !== "handcrafted_plot98")) {
            ...
            receiveBuildingData(buildingData);
        } else if (isProject3Text && (!window.lastBuildingData || window.lastBuildingData.source !== "handcrafted_project3")) {
            setStatus("OCR detected Project 3 Floor Plan. Loading detailed 3D structure...");
            const currentFloors = window.lastBuildingData ? window.lastBuildingData.floors : 1;
            const buildingData = generateDetailedProject3Data(currentFloors);
            ...
            const mockOcrData = {
                roomNames,
                dimensions,
                floorNumbers: [String(currentFloors)],
                areaValues: buildingData.rooms.map(r => `${(r.width * r.depth).toFixed(1)} sq m`),
                fireSafetyLabels: ["Extinguisher located near core"],
                stairLabels: ["Main Staircase Center"],
                liftLabels: ["Double Elevator Core"],
...
function showPlanImage(url, fileName) {
    ...
    img.onload = () => {
        currentVisionImageWidth = img.naturalWidth || img.width || 1024;
        currentVisionImageHeight = img.naturalHeight || img.height || 768;
    };
    img.src = url;
    if (meta && fileName) meta.innerText = "Source: " + fileName;
    panel.style.display = "block";

    // Set Vision Image Base64 for Gemini AI Assistant
    if (url.startsWith("data:image")) {
        currentVisionImageBase64 = url;
        ...
        tempImg.onload = () => {
            try {
                const canvas = document.createElement("canvas");
                canvas.width = Math.min(1024, tempImg.naturalWidth || tempImg.width);
                canvas.height = Math.min(1024, tempImg.naturalHeight || tempImg.height);
                const ctx = canvas.getContext("2d");
                ctx.drawImage(tempImg, 0, 0, canvas.width, canvas.height);
                currentVisionImageBase64 = canvas.toDataURL("image/jpeg", 0.85);
                currentVisionImageWidth = tempImg.naturalWidth || tempImg.width || 1024;
                currentVisionImageHeight = tempImg.naturalHeight || tempImg.height || 768;
                showVisionBadge(true);
            } catch (e) {
                console.warn("Could not encode image for Gemini vision:", e);
            }
        };
        tempImg.src = url;
    }
...
function showVisionBadge(visible) {
    const badge = document.getElementById("aiVisionBadge");
...
////////////////////////////////////////////////////////
// Gemini AI Assistant & 3D Command Center Integration
////////////////////////////////////////////////////////

function executeAiAction(name, args) {
    console.log("AI Executing Action:", name, args);
    let summary = '';

    switch (name) {
        case 'setLayerVisibility': {
            if (args.pipes !== undefined) {
                pipesGroup.visible = args.pipes;
                const el = document.getElementById("togglePipes");
                if (el) el.checked = args.pipes;
            }
            if (args.electrical !== undefined) {
                electricalGroup.visible = args.electrical;
                const el = document.getElementById("toggleElectrical");
                if (el) el.checked = args.electrical;
            }
            if (args.escape !== undefined) {
                escapeRoutesGroup.visible = args.escape;
                const el = document.getElementById("toggleEscape");
                if (el) el.checked = args.escape;
            }
            if (args.windows !== undefined) {
                windowsGroup.visible = args.windows;
                const el = document.getElementById("toggleWindows");
                if (el) el.checked = args.windows;
            }
            if (args.multiFloor !== undefined) {
                const el = document.getElementById("toggleMultiFloor");
                if (el) el.checked = args.multiFloor;
                if (window.lastBuildingData) {
                    receiveBuildingData(window.lastBuildingData);
                }
            }
            summary = 'Updated layer visibility.';
            break;
        }
        case 'highlightFireZone': {
            const { blockIdx, roomIdx, floorIdx } = args;
            const blockEl = document.getElementById('blockIndex');
            const roomEl = document.getElementById('roomIndex');
            const floorEl = document.getElementById('floorIndex');
            if (blockEl) blockEl.value = String(blockIdx);
            if (roomEl) roomEl.value = String(roomIdx);
            if (floorEl) floorEl.value = String(floorIdx);
            ...
            if (roomObj) {
                ...
                highlightFireZones(roomIdx, floorIdx, blockIdx);
                updateClickedTooltip();
                summary = `Highlighted fire zone: Block ${blockIdx}, Floor ${floorIdx}, Room ${roomIdx}.`;
            } else {
                summary = `Room not found for Block ${blockIdx}, Floor ${floorIdx}, Room ${roomIdx}.`;
            }
            break;
        }
        case 'toggleWalkthrough': {
            const walkBtn = document.getElementById("walkthroughBtn");
            const shouldEnable = args.enable !== undefined ? args.enable : !isWalkthroughMode;
            if (shouldEnable !== isWalkthroughMode && walkBtn) {
                walkBtn.click();
            }
            summary = shouldEnable ? 'Entered Walkthrough mode.' : 'Exited Walkthrough mode.';
            break;
        }
        case 'setCameraPreset': {
            ...
            if (preset === 'isometric') {
                ...
                camera.position.set(180, 20, 0);
                controls.target.set(0, 20, 0);
            }
            controls.update();
            summary = `Camera perspective set to ${preset}.`;
            break;
        }
        case 'setFloorsCount': {
            const count = Math.max(1, Math.min(20, args.count));
            const floorsInput = document.getElementById("floorsOverride");
            if (floorsInput) floorsInput.value = count;
            if (window.lastBuildingData) {
                receiveBuildingData(window.lastBuildingData);
            }
            summary = `Building floor count set to ${count}.`;
            break;
        }
        case 'clearBuildingScene': {
            clearScene();
            summary = 'Cleared 3D building scene.';
            break;
        }
        case 'renderBuildingLayout': {
            ...
            const mockOcrData = {
                ...
                stairLabels: [],
                liftLabels: [],
                rawOCRText: "Parsed via Gemini Chat AI"
            };
            window.lastOCRData = mockOcrData;

            receiveBuildingData(buildingData);
            summary = `Successfully rendered 3D building layout with ${rooms.length} rooms.`;
            break;
        }
        default:
...
function setupAiAssistantUI() {
    ...
    const aiRemoveVisionBtn = document.getElementById("aiRemoveVisionBtn");
    ...
    if (saveApiKeyBtn && apiKeyInput && apiKeyModal) {
        saveApiKeyBtn.addEventListener("click", () => {
            ...
            setApiKey(key);
            apiKeyModal.style.display = "none";
            appendAiMessage("system", key.trim() ? "API key saved." : "API key cleared.");
        });
    }

    if (aiRemoveVisionBtn) {
        aiRemoveVisionBtn.addEventListener("click", () => {
            currentVisionImageBase64 = null;
    ...
    chips.forEach(chip => {
        chip.addEventListener("click", () => {
            ...
            if (action === "analyze") {
                handleUserChatMessage("Analyze the uploaded floor plan image. Generate the 3D model layout according to the scanned image using renderBuildingLayout.");
            } else if (action === "pipes") {
                handleUserChatMessage("Show water pipes layer in 3D");
            } else if (action === "electrical") {
                handleUserChatMessage("Show electrical wiring lines in 3D");
            } else if (action === "fire") {
                handleUserChatMessage("Highlight fire hazard in Block 0, Room 1, Floor 0");
            } else if (action === "walkthrough") {
                handleUserChatMessage("Enter 3D walkthrough mode");
            }
        });
    });
...
async function handleUserChatMessage(promptText) {
    appendAiMessage("user", promptText);

    const buildingContext = {
        hasBuilding: !!window.lastBuildingData,
        floors: window.lastBuildingData?.floors || 1,
        roomCount: roomMeshes.length,
        layers: {
            pipes: pipesGroup.visible,
            electrical: electricalGroup.visible,
            escape: escapeRoutesGroup.visible,
            windows: windowsGroup.visible
        },
        walkthroughMode: isWalkthroughMode
    };

    const isApiKeySet = !!getApiKey();
    if (!isApiKeySet) {
        appendAiMessage("error", "API Key is missing. Click 🔑 in the AI Chat panel header to save your key.");
        return;
    }

    const typingMsg = appendAiMessage("assistant", "Thinking...");

    let adjustedPrompt = promptText;
    if (currentVisionImageBase64) {
        adjustedPrompt += `\n\n[Note: The attached floor plan image dimensions are ${currentVisionImageWidth} x ${currentVisionImageHeight} pixels. When parsing or generating a 3D layout from this image, you MUST use ratio logic to scale the room center coordinates (x, z) and dimensions (width, depth) proportionally so that the 3D model aspect ratio matches the image aspect ratio (${(currentVisionImageWidth / currentVisionImageHeight).toFixed(3)}) exactly, preventing it from being stretched or disproportionate. Scale the maximum dimension (width or depth) to 80 meters, and scale the other dimension proportionally. Center the coordinates around (0, 0).]`;
    }

    try {
        const response = await queryGemini({
            prompt: adjustedPrompt,
            imageBase64: currentVisionImageBase64,
            buildingContext: buildingContext,
            history: chatHistory,
            onActionExecute: (name, args) => {
                const actionSummary = executeAiAction(name, args);
                appendAiMessage("system", `⚡ Action Executed: ${actionSummary}`);
        ...
        appendAiMessage("error", "AI Error: " + err.message);
    }
}
